...
public class SehirlerVePlakalar {
	...
	public static void main(String[] args) {
		...
		*/
		...
		String arraySehirler[] =sehirleriAyir(sehirler);
		for (int i = 0; i < arraySehirler.length; i++) {          //yeni arrayi ekrana yazdırma
			System.out.println(arraySehirler[i]);
		}
		plakalariAyir(arraySehirler);
		

	} 
	public static String[] sehirleriAyir(String sehirler) {         
		    
		String[] arraySehirler=sehirler.split(";");                     //stringi arraye çevirme
		return arraySehirler;
	}
	public static void plakalariAyir(String[] ayrilcak) {               
		
		for(int i = 0; i<ayrilcak.length;i++) {
			String[] plakaSehirler = ayrilcak[i].split("-");                   //şehirle plakasını ayırma
			System.out.println(plakaSehirler[1]+" adlı şehrin plaka kodu : "+plakaSehirler[0]);
		}
